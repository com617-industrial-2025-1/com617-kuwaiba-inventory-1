# Kuwaiba With Overlay Data

In this proof of concept, Kuwaiba is provided with a simple base model which has some examples and the import script already provided.

If you want to play with a pre-populated bitternePark model which has example GPON circuits, move the zip file 
`imported-bitterne-park-example-data.zip` to the `data-zip` folder and rename it to replace the existing `data.zip` file.

To import this model, shutdown and remove the old data from Kuwaiba and restart. (Remember to use the correct profile)
...
This configuration of a Kuwaiba  docker compose project allows you to experiment with Kuwaiba and store a backup of your database externally within your docker compose project.
...
(If data.zip is not present, the default Kuwaiba model from the container will be used).
